...
- **가사 자동 인식 (AI)** — 모든 후보 페이지를 먼저 악보/비악보로 분류하고, 악보로 확인된
  **새 찬양**만 스캔 이미지에서 제목·파트
  ...
  - **웹 가사 대조 (새 찬양 전용)** — 인식의 마지막 단계입니다. 콘티가 들어오면 ① 제목 인식 →
    ...
    대조는 **줄 단위**로 이루어집니다. 파트를 통째로 웹 가사로 바꾸지 않고, 인식한 줄과 웹의 줄이
    **같은 줄로 읽힐 때만** 웹의 표기를 채택합니다 — 철자 교정이지 문장 교체가 아닙니다.
    채택한 줄은 **웹 페이지에 실린 그대로** 들어갑니다. 웹에
    대응하는 줄이 없으면 모델이 읽은 그대로 둡니다 (그 악보의 편곡에만 있는 줄일 수 있습니다).
    충분히 대조가 맞은 파트에 한해, 모델이 놓친 **마지막 줄들**은 웹에서 이어 붙입니다 — 끝줄
    한두 줄이 잘리는 것이 OCR에서 가장 흔한 누락이기 때문입니다.
    진행 순서에는 있는데 어느 모델도 읽지 못한 파트만 웹에서 채워 넣고, 콘티가 부르지 않는
    파트는 가져오지 않습니다.
    **라이브러리에 이미 있는 곡은 이 단계를 건너뜁니다** — 저장된 가사는 그대로 사용합니다.
    - **벅스는 기본적으로 읽지 않습니다** — 상업 서비스의 페이지를 수집할지는 코드가 아니라
      배포 정책이 정할 일이라, Worker 환경 변수 `BUGS_SCRAPING_ALLOWED`가 켜져 있을 때만
      읽습니다. 꺼져 있으면 검색 결과를 **링크로만** 보여 주고 페이지는 가져오지 않습니다.
