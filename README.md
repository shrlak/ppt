...
- **가사 자동 인식 (AI)** — 라이브러리에 없는 **새 찬양**은 스캔된 악보 이미지에서 제목·파트
  ...
  자동으로 **Hugging Face**로 전환됩니다 (브라우저 자체 OCR은 사용하지 않습니다).
  ...
  - **Hugging Face (자동 대체)** — Gemini의 무료 한도가 소진되거나 호출이 실패하면 자동으로 이어서
    시도하는 대체 엔진입니다.
...
- **AI 사용량 페이지** — 헤더의 **사용량** 버튼(관리자 버튼 옆)을 누르면 공유 프록시를 통해 사용한
  Gemini와 Hugging Face의 모델별 무료 한도 사용량을 막대로 확인할 수 있습니다. Gemini는 일일 요청
  수와 실제 응답 토큰을, Hugging Face는 월간 무료 크레딧 대비 계산 시간 기반 추정 비용을 표시합니다.
