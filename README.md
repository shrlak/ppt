...
- **가사 자동 인식 (AI)** — 모든 후보 페이지를 먼저 악보/비악보로 분류하고, 악보로 확인된
  **새 찬양**만 스캔 이미지에서 제목·파트
  ...
  - **웹 가사 대조 (새 찬양 전용)** — 인식의 마지막 단계입니다. 콘티가 들어오면 ① 제목 인식 →
    ② 악보 가사 인식 → ③ **인식된 제목으로 웹에서 가사를 검색**해 순서대로 가져온 뒤 →
    ④ **절·후렴·프리코러스·브릿지** 파트를 붙여 가사 편집기에 채웁니다.
    ...
    **같은 줄로 읽힐 때만** 웹의 표기를 채택합니다 — 철자 교정이지 문장 교체가 아닙니다. 웹에
  ...
  - **한국어 맞춤법 정리 (새 찬양 전용)** — 편집기에 들어가는 가사를 자모 결합(NFC), 음표에
    맞춰 쪼개진 하이픈 잇기, 그 과정에서 떨어져 나온 조사 붙이기, 문장 부호 간격, 그리고
    어떻게 읽어도 틀린 표기(`할께`→`할게`, `됬`→`됐` 등)만 정리합니다. **작사자의 단어 선택은
    바꾸지 않으며**, 라이브러리 가사나 사용자가 직접 입력한 가사에는 적용되지 않습니다.
  - **동시 실행 모델 (모든 기기 공유)** — 인식할 때마다 아래 모델을 모두 한꺼번에 실행합니다.
    **주 모델**은 무료 티어에서 가장 성능이 좋은 **Gemini 3.6 Flash**와 **Gemini 3.5 Flash**이고,
    나머지 **NVIDIA Nemotron Nano 12B VL**(문서·OCR 특화)과 **NVIDIA Nemotron 3 Nano Omni 30B**
    (추론형)은 두 주 모델의 답을 보조하는 보조 모델입니다. 풀은 이 **네 개**가 전부입니다.
    성능이 떨어지거나
    구버전인 모델은 풀에 넣지 않습니다 — 틀린 답이 하나 늘면 줄 단위 다수결이 흔들리고 무료
    한도만 소모하기 때문입니다. OpenRouter
    모델은 모두 Cloudflare Worker를 통해 호출되며 OpenRouter 키가 브라우저에 노출되지 않습니다.
    OpenRouter 무료 엔드포인트는 입력 이미지와 출력이 공급자에 기록될 수 있으므로 민감한
    악보에는 사용하지 마세요.
    각 페이지는 우선순위가 가장 높은 모델(주 모델 우선)의 정상 결과를 사용하고, 그 모델이
    놓친 항목은 보조 모델의 답으로 채웁니다. 모든 공급자의 무료 요청 한도가 한 번의 인식에서
    함께 사용됩니다.
  - **어려운 페이지도 전체 모델 동시 사용** — 일괄 인식이 읽지 못한 페이지는 더 높은 해상도로
    다시 렌더링한 뒤 전체 모델에 동시에 요청해 가장 먼저 읽어낸 결과를 사용합니다.
