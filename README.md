...
- **가사 자동 인식 (AI)** — 모든 후보 페이지를 먼저 악보/비악보로 분류하고, 악보로 확인된
  **새 찬양**만 스캔 이미지에서 제목·파트
  ...
  - **웹 가사 대조 (새 찬양 전용)** — 인식의 마지막 단계입니다. 콘티가 들어오면 ① 제목 인식 →
    ...
    담당합니다. 진행 순서에는 있는데 어느 모델도 읽지 못한 파트만 웹에서 채워 넣고, 콘티가
    부르지 않는 파트는 가져오지 않습니다.
  ...
  - **동시 실행 모델 (모든 기기 공유)** — 인식할 때마다 아래 모델을 모두 한꺼번에 실행합니다.
    **주 모델**은 무료 티어에서 가장 성능이 좋은 **Gemini 3.6 Flash**와 **Gemini 3.5 Flash**이고,
    나머지 **NVIDIA Nemotron Nano 12B VL**(문서·OCR 특화)·**NVIDIA Nemotron 3 Nano Omni 30B**
    (추론형)·**Gemma 4 26B A4B**는 두 주 모델의 답을 보조하는 보조 모델입니다. 성능이 떨어지거나
...
- **AI 사용량 페이지** — 헤더의 **사용량** 버튼(관리자 버튼 옆)을 누르면 공유 프록시를 통해 사용한
  ...
  Gemini와 OpenRouter는 일일 요청 수와 실제 응답 토큰을, Hugging Face는
  월간 무료 크레딧 대비 계산 시간 기반 추정 비용을 표시합니다.
