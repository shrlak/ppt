...
다운로드 단계의 **라이브러리에 저장** 버튼을 누르면 방금 만든 PPT를 콘티 PDF·설교 PPT와
함께 공유 서버와 이 브라우저의 오프라인 캐시에 보관합니다. 이때 그 PPT를 만든 입력 내용
(찬양 가사·순서, 성경 본문·설교 제목·번역본, 광고 원문)도 함께 저장됩니다. 헤더의
**라이브러리** 버튼으로 저장된 PPT 목록을 열어 어느 기기에서나 다시 다운로드·편집·삭제할
수 있습니다.

**편집**을 누르면 그 PPT를 만들 때의 입력 내용 그대로 찬양 → 성경 말씀 → 설교 → 광고 →
다운로드 5단계가 다시 열립니다. 내용을 고치고 다시 저장하면 같은 항목이 갱신되며, 파일명을
바꿔도 항목이 새로 생기지 않고 이름만 바뀝니다. 다운로드 단계의 **새 항목으로 저장** 버튼을
누르면 연결을 끊고 별개의 항목으로 저장합니다. 입력 내용이 함께 저장되기 전에 만들어진
항목은 콘티 PDF와 설교 PPT만 복원되고, 콘티는 새로 업로드한 것처럼 다시 분석됩니다.
...
- **가사 자동 인식 (AI)** — 모든 후보 페이지를 먼저 악보/비악보로 분류하고, 악보로 확인된
  **새 찬양**만 스캔 이미지에서 제목·파트
  ...
  채워 줍니다. Gemini·OpenRouter·Hugging Face의 모든 등록 모델을 **동시에 실행**하고, 페이지마다
  우선순위가 가장 높은 모델의 정상 결과를 사용합니다 (브라우저 자체 OCR은 사용하지 않습니다).
  - **진행률 표시** — 인식이 도는 동안 각 곡 카드에 현재 단계(악보 준비 → 제목 확인 → 가사 인식
    → 재시도)와 **퍼센트(%) 진행률 막대**가 실시간으로 표시됩니다.
  - **Gemini (동시 모델 풀)** — 악보 이미지를 Gemini에 보내 구조화된 결과(JSON)를 받습니다.
    무료 API 한도가 있는 **Gemini 2.5 Flash**와 **Gemini 2.0 Flash**도 다른 공급자 모델과 동시에
  ...
  - **동시 실행 모델 (모든 기기 공유)** — 인식할 때마다 아래 모델을 모두 한꺼번에 실행합니다.
    **주 모델**은 **Gemini 2.5 Flash**와 **OpenRouter 무료 NVIDIA Nemotron Nano 12B VL**
    (문서·OCR 특화)이고, 나머지 **Gemini 2.0 Flash**·**Gemma 4 31B**·**Gemma 3 27B**·
    **Hugging Face Qwen2-VL**은 두 주 모델의 답을 보조하는 보조 모델입니다. OpenRouter
...
- **AI 사용량 페이지** — 헤더의 **사용량** 버튼(관리자 버튼 옆)을 누르면 공유 프록시를 통해 사용한
  Gemini·OpenRouter·Hugging Face의 모델별 무료 한도 사용량을 막대로 확인할 수 있습니다.
