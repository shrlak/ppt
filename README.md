...
첫 화면 맨 위의 **일괄 업로드** 상자에 찬양 콘티 PDF, 설교 PPT, 표지·마무리·성경 말씀
템플릿(.pptx)을 한꺼번에 끌어다 놓으면, 파일 이름으로 자동 구분해서 각자 맞는 자리에
채워 줍니다 (예: `설교.pptx` → 설교 PPT, `표지템플릿.pptx` → Front 템플릿). 이름으로
구분이 안 되는 파일은 "확인 필요" 표시와 함께 직접 역할을 지정할 수 있습니다. 물론 각
단계의 개별 업로드 버튼과 관리자 설정으로도 언제든 따로 올리거나 바꿀 수 있습니다.
...
- **가사 자동 인식 (AI)** — 모든 후보 페이지를 먼저 악보/비악보로 분류하고, 악보로 확인된
  **새 찬양**만 스캔 이미지에서 제목·파트
  ...
  가장 먼저 도착한 정상 결과를 사용합니다 (브라우저 자체 OCR은 사용하지 않습니다).
  ...
  - **동시 실행 모델 (모든 기기 공유)** — 인식할 때마다 아래 모델을 모두 한꺼번에 실행합니다.
    사용 가능한 모델: Gemini 2.5 Flash / 2.0 Flash,
    OpenRouter 무료 NVIDIA Nemotron Nano 12B VL(문서·OCR 특화)·Gemma 4 31B·Gemma 3 27B,
    그리고 Hugging Face Qwen2-VL입니다. OpenRouter 모델은 모두 Cloudflare Worker를 통해
    호출되며 OpenRouter 키가 브라우저에 노출되지 않습니다.
    OpenRouter 무료 엔드포인트는 입력 이미지와 출력이 공급자에 기록될 수 있으므로 민감한
    악보에는 사용하지 마세요.
    각 페이지는 가장 먼저 완료된 정상 결과를 사용하고, 먼저 끝난 모델이 놓친 페이지는 뒤이어 끝난
    모델이 채웁니다. 모든 공급자의 무료 요청 한도가 한 번의 인식에서 함께 사용됩니다.
