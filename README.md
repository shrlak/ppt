...
1. **주일예배 (대청부)** — 이 페이지의 6단계 화면 (`?service=sunday`로 바로 열 수 있습니다)
2. **수요예배** — 곡 PPT·악보 사진으로 만드는 수요예배 PPT와 썸네일 (`wednesday.html`)
3. **찬양집회** — 한글·영어 제목과 가사가 함께 들어가는 찬양집회 PPT (`praise.html`)
4. **수련회** — 수련회 디자인으로 집회마다 한 PPT씩 (`retreat.html`)

어느 생성기에서든 왼쪽 초록색 사이드바에서 다른 예배 종류로 바로 옮겨 가고, 지금 생성기의
단계로 건너뛰고, 라이브러리·사용량·관리자 도구를 엽니다. 휴대폰처럼 좁은 화면에서는 이 사이드바가
페이지 맨 위로 접힙니다. `이전`/`다음` 버튼은 화면 아래에 떠 있어 긴 단계에서도 스크롤하지 않고
누를 수 있습니다.
...
페이지 제목 오른쪽의 **편집기 보기** 버튼을 누르면 단계별 화면 대신 PowerPoint 편집기 같은
화면으로 전환됩니다. 넓은 화면이 필요하므로 사이드바는 아이콘만 남기고 접힙니다. 왼쪽에 최종 PPT의 전체 슬라이드 목록(Front → 찬양 → 기도 → 성경 말씀 → 설교 →
...
저장됩니다. 사이드바의 **라이브러리** 버튼으로 저장된 PPT 목록을 열어 어느 기기에서나 다시
다운로드·편집·삭제할 수 있습니다.
...
- **AI 사용량 페이지** — 사이드바의 **사용량** 버튼(관리자 버튼 위)을 누르면 공유 프록시를 통해 사용한
...
- **번역본** — 한국어(개역개정·개역한글·새번역·쉬운성경·현대인의성경)와 영어(ESV·NIV·NASB·KJV) 중 최대 2개를 함께 표시할 수
  있습니다.
...
- 성경 본문 데이터: `public/bible-text/*.json` (번역본별 전체 본문, 실제 사용 시에만 지연 로드).
  ...
  - 쉬운성경은 아가페출판사 본문을 갓피아(godpia.com) 성경 읽기에서 절 번호 기준으로 같은 방식으로 옮긴 것입니다.
    묶어 인쇄한 절(삿20:22-23, 삼하4:6-7, 왕상8:41-42)은 첫 번호 자리에 본문과 빈 문자열을 두고, 빠진 절은
    "(없음)"입니다.
  - 현대인의성경은 생명의말씀사 본문을 같은 갓피아 성경 읽기에서 같은 방식으로 옮긴 것입니다. 여러 절을 한 문장으로
    풀어 쓴 곳이 많아(룻4:18-22, 민7:12-83 등) 모두 첫 번호 자리에 본문과 빈 문자열로 묶습니다. 갓피아가 뒤 번호를
    표시하지 않거나 "(없음)"으로 적은 곳(민11:4-5, 창25:7-8 등)도 앞 절이 그 내용을 담고 있으므로 같이 묶고,
    갓피아에 빠져 있는 계5:14는 같은 판의 YouVersion(KLB) 본문으로 채웠습니다.
