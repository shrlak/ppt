...
- **가사 자동 인식 (AI)** — 라이브러리에 없는 **새 찬양**은 스캔된 악보 이미지에서 제목·파트
  ...
  채워 줍니다. **Gemini를 항상 우선 사용**하며, Gemini의 토큰·요청 한도가 소진되거나 실패하면
  자동으로 **NVIDIA → Hugging Face** 순으로 전환됩니다 (브라우저 자체 OCR은 사용하지 않습니다).
  - **진행률 표시** — 인식이 도는 동안 각 곡 카드에 현재 단계(악보 준비 → 제목 확인 → 가사 인식
    → 재시도)와 **퍼센트(%) 진행률 막대**가 실시간으로 표시됩니다.
  - **Gemini (기본)** — 악보 이미지를 Gemini에 보내 구조화된 결과(JSON)를 받습니다. 가사
    인식은 **Gemini 2.5 Pro를 먼저** 시도하고, 한도가 소진되면 자동으로 Flash로 내려갑니다
    (빠른 제목 확인은 Flash 사용). 주 1회 콘티 정도는 무료 한도 안에서 처리됩니다. 인식용 악보
  ...
  - **모델 우선순위 (모든 기기 공유)** — 인식에 쓸 모델과 순서를 **관리자 설정**에서 바꿀 수
    있습니다. 사용 가능한 모델: Gemini 2.5 Pro / 2.5 Flash / 2.0 Flash,
    [build.nvidia.com](https://build.nvidia.com/models)의 NVIDIA Nemotron Nano 12B VL(문서·OCR
    특화)·Llama 3.2 90B Vision·Gemma 3 27B·Phi-4 Multimodal, 그리고 Hugging Face Qwen2-VL.
    위에서부터 차례로 시도하고, 실패하거나 무료 한도가 차면 자동으로 다음 모델로 넘어갑니다.
    변경한 우선순위는 공유 서버에 저장되어 **모든 기기·모든 사용자에게 동일하게** 적용되고, 다음
    변경 전까지 유지됩니다.
  - **어려운 페이지는 여러 모델 동시 사용** — 일괄 인식이 읽지 못한 페이지는 우선순위 상위 모델
    여러 개에 **동시에** 다시 요청해 가장 먼저 읽어낸 결과를 사용합니다.
  - **자동 재시도·구출 인식** — 일시적 오류(429/5xx/네트워크)는 잠시 후 한 번 더 시도하고,
    일괄 인식에서 가사를 읽지 못한 페이지는 **곡별로 다시 인식**해 한 페이지 문제가 전체 콘티를
    망치지 않습니다. 빈 응답은 성공으로 치지 않고 다음 엔진으로 넘어갑니다.
...
- **AI 사용량 페이지** — 헤더의 **사용량** 버튼(관리자 버튼 옆)을 누르면 공유 프록시를 통해 사용한
  Gemini·NVIDIA·Hugging Face의 모델별 무료 한도 사용량을 막대로 확인할 수 있습니다. Gemini는 일일
  요청 수와 실제 응답 토큰을, NVIDIA는 무료 크레딧(요청 1회 = 1크레딧) 사용량을, Hugging Face는
