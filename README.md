...
- 매번 새 항목이 생기지 않고 **같은 항목이 계속 갱신**됩니다. 파일명을 바꾸면 항목 이름도 함께
  바뀝니다(사본이 생기지 않습니다).
- 슬라이드 내용이 실제로 달라진 편집만 저장합니다. 라이브러리에서 불러오기만 하거나, 화면을
  전환하거나, 같은 내용을 다시 입력한 경우에는 다시 올리지 않습니다.
...
- **제목이 라이브러리에 있으면 가사 인식을 건너뜁니다** — 콘티가 들어오면 악보에서 **제목만** 먼저
  ...
  찬양만 악보에서 가사를 읽습니다 — 이미 저장해 둔 곡을 다시 읽느라 무료 한도를 쓰지 않습니다.
  - **제목이 확실할 때만 건너뜁니다** — 모델들이 제목을 서로 다르게 읽었다면(합의 90% 미만)
    건너뛰지 않고 악보 가사를 읽은 뒤, **읽은 가사까지 같을 때만** 저장본을 씁니다. 제목만 비슷한
    다른 곡의 가사가 슬쩍 들어오지 않도록 하기 위해서입니다. 두 가사가 다르면 악보에서 읽은 쪽이
    남습니다. 콘티 표지에 적힌 제목은 이미지가 아니라 PDF 텍스트라 그대로 믿습니다.
  - 저장된 가사는 **사용자가 확인한 것만**(초안 제외) 쓰이고, 아티스트를 아는 경우 아티스트까지
    같아야 합니다.
...
- **검증된 교정에서 배우기** — 자동 인식 결과는 **초안**입니다. 사람이 확인하기 전까지는 다음
  ...
  - **초안 / 검증됨 / 수정본** — 카드의 **라이브러리에 저장**을 눌렀을 때만 정답이 됩니다.
    ...
    증거이기 때문입니다.
  - **재사용은 정답만** — 다음 콘티에서 인식을 건너뛰고 저장된 가사를 쓰는 것은 **검증됨/수정본**
    뿐입니다. 초안으로 인식을 건너뛰면 확인되지 않은 한 번의 오독이 영구히 굳어 버립니다.
